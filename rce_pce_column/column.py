...
class column :
    ...
    def __init__( self , input_data , inits_extra=None , verbose=False ):
        ...
        if type( input_data ) is dict:
            ...
            for key in keys:
                self.I[key] = nc_fid.variables[key][:]
                self.D[key] = nc_fid.variables[key][:]
                extra = []
    ...
    def ozone_chemistry( self ):
        ...
        self.D['nm'] = ( 6.022E+23 * self.D['lyP'] )                  \
        ...
        # densities
        o2 = self.D['nm']*self.D['lyO2']
        # column densities ( molecules / meter^2 )
        nm_col = np.diff(self.D['lvZ']*1000) * self.D['nm']
        ...
        self.D['j3'] = np.sum( o3_tr * self.flux_j3 * self.sigma_j3_o3 , axis=0 ) 
        ...
        o_to_o3_ratio = self.D['j3'] / (k2*o2*self.D['nm']) # O/O3
        ...
        lyNO  = no_to_nox_ratio * self.D['lyNOx']
        lyNO2 = (1-no_to_nox_ratio) * self.D['lyNOx']
        ...
        sink_nox = (b4*lyNO + b3*lyNO2*o_to_o3_ratio)*self.D['nm']*86400 # ppv/day
        ...
        source   = 2*self.D['j2']*o2/self.D['nm']*3600*24 # ppv / day
        sink_ox  = 2*k3*self.D['j3']/(k2*o2)*3600*24 # ppv / day
        #sink_oh  = hk1*self.lyOH*0.0
        #sink_ho2 = hk2*self.lyHO2*0.0
        #sink_hox = (sink_oh+sink_ho2)*3600*24 # ppv/day
        ...
        self.D['lyO'] += self.D['timestepsize'] * \
                ( source\
                - sink_ox*self.D['lyO']**2 \
                - sink_nox*self.D['lyO'] )

        #self.sink_hox = sink_hox*self.D['lyO']
    ...
    def set_pressure( self ):
        ...
        self.D['lvP'][1:] = self.D['lvP'][0]*np.exp(
                - 9.81 / 287.058 * np.cumsum( 
                    np.diff( self.D['lvZ'] * 1000 ) / self.D['lyT']
                    )
                )
    ...
    def run_rad( self ):
        ...
        input_lines.append(
                ...
                +3*' '                                                          
                +('%2i' % -1 ) #ISOLVAR                                          
                +('%10.4f' % 0.0) #SCON - not sure if this is right             
        ...
        input_lines.append(
                ...
                +('%10.4f' %  self.D['lyT'][i])                    
                +' '*23
                +('%8.3f'  % (self.D['lvP'][i]/100))                
                +('%7.2f'  %  self.D['lvT'][i])                     
                +' '*7
                +('%8.3f'  % (self.D['lvP'][i+1]/100))              
                +('%7.2f'  %  self.D['lvT'][i+1])                   
                +'\n'                                               
                +('%10.3e' %  self.D['lyW'][i])                     
        ...
        if abs(self.D['sza'])<90: 
            ...
            self.D['lyHS'] = sw_output[-1:0:-1,2][:194]

        else:
            self.D['lyHS'] = self.D['lyHL']*0.0


        if np.any(np.isnan( self.D['lyHL'] )): raise Exception("lyHL is nan!")
        if np.any(np.isnan( self.D['lyHS'] )): raise Exception("lyHS is nan!")
    ...
    def quasi_analytical_ozone( self ):
        ...
        guess = ( np.sqrt( self.O['sink_nox']**2 \
                + 4 * self.O['source'] * self.O['sink_ox'] \
                ) - self.O['sink_nox'] ) / ( 2 * self.O['sink_ox'] )
    ...
    def set_broad( self ):
        ...
        self.D['lyBroad'] = ( self.D['lvP'][:-1]-self.D['lvP'][1:] ) \
